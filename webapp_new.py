# -*- coding: utf-8 -*-
"""
Created on Mon May 27 16:56:38 2024
...
dict_down={}
name_col=[]
for country in dict_sce.keys():
    df_sub=dict_sce[country][1]
    if len(df_sub)>0:
        df_sub_p = df_sub.sort_index(ascending=False)
        df_sub_p.index=df_sub_p.index*100
        df_sub_p = df_sub_p *(hist_df.loc[:,country].max()-hist_df.loc[:,country].min()) + hist_df.loc[:,country].min()
        df_sub = df_sub_p.loc[df_sub_p.index.max(),:]
        if type(df_sub)!=pd.Series:
            df_sub=df_sub.mean()
        l_pred.append(df_sub)
        df_sub_p=df_sub_p.T
        col_n = ['Date']+['Scenario '+str(i+1)+':'+str(int(col))+'%' for i,col in enumerate(df_sub_p.columns)]
        df_sub_p.reset_index(inplace=True)
        df_sub_p.columns = col_n
        dict_down[country]=df_sub_p
        name_col.append(country)
l_pred=pd.concat(l_pred,axis=1)     
l_pred.reset_index(inplace=True)  
l_pred.columns= ['Date']+name_col
l_pred_pres=l_pred.iloc[:,:10]
l_pred_pres=l_pred_pres.round(2)
l_pred_pres.iloc[:, 0] = pd.to_datetime(l_pred_pres.iloc[:, 0])
l_pred_pres.iloc[:, 0] = l_pred_pres.iloc[:, 0].dt.strftime('%B %Y')
                ...
                dbc.Row([
                    dbc.Col(html.Div([
                        ...
                        dbc.Nav([
                            ...
                            dbc.NavLink("Download Data", href="/download", style={'color': '#555'}),
...
down_layout = html.Div([
    dbc.Container([
            dbc.Row([
                dbc.Col(html.Div([
                    html.H2("Download the Data", style={'textAlign': 'left'}),
                    dbc.Nav([
                        dbc.NavLink("Risk Map", href="/home", style={'color': '#555'}),
                        dbc.NavLink("Monthly Report", href="/report", style={'color': '#555'}),
                        dbc.NavLink("Download Data", href="/download", style={'color': '#555'}),
    ...
    html.Div([
        html.H1("Get the Data", style={'marginBottom':20,'textAlign': 'center'}),
        dbc.Tabs([
            dbc.Tab(
                dbc.Card(dbc.CardBody([
                html.H4('Global Prediction'),
                dcc.Markdown('''The global prediction is the country based forecast for the next 6 months. 
                             It represents the scenario with the highest probability. The country specific scenarios are available in 'Country Specific' section.''',style={'width':'70vw','marginBottom':'5vh'}),
                html.Div(dash_table.DataTable(data=l_pred_pres.to_dict('records'),columns=[{'id': c, 'name': c} for c in l_pred_pres.columns],style_cell={'textAlign': 'left'}),style={'width': '60vw','margin':'auto','marginBottom':'5vh'}),
                html.Div([dbc.Button("CSV Download", outline=True, color="danger",id='btn1'),
                          dcc.Download(id="dl1")
                          ]),
                ],style={'width':'90vw','marginLeft':'5vw'}),className="mt-3"),label='Global Prediction',style={'width':'90vw','marginLeft':'5vw'},active_label_style={"color": "#df2226"}),
           dbc.Tab(
               dbc.Card(dbc.CardBody([
                   html.H4('Country Specific'),
                   dcc.Markdown('''The country specific is specific country scenarios for the next 6 months. 
                                Each column is a scenario with its correpsonding probability. The general forecasts are available in 'Global Prediction' section.''',style={'width':'70vw','marginBottom':'5vh'}),
                   dcc.Markdown('''Please select the country :''',style={'width':'70vw'}),
                   dcc.Dropdown(l_pred.columns[1:], l_pred.columns[1], id='drop-c',style={'width':'20vw','marginBottom':'5vh'}),
                   html.Div(id='tbl'),
                   dcc.Store(id='c_store',storage_type='session'),
                   html.Div([dbc.Button("CSV Download", outline=True, color="danger",id='btn2'),
                             dcc.Download(id="dl2")])
               ],style={'width':'90vw','marginLeft':'5vw'}),className="mt-3"),label='Country Specific',style={'width':'90vw','marginLeft':'5vw'},active_label_style={"color": "#df2226"})
           ])
        ])
    ])

@webapp.callback(Output('tbl', 'children'),
                 Output('c_store','data'),
              Input("drop-c",'value'))
...
def print_tab(val2):
    ...
    df.iloc[:, 0] = df.iloc[:, 0].dt.strftime('%B %Y')
    return html.Div(dash_table.DataTable(data=df.to_dict('records'),columns=[{'id': c, 'name': c} for c in df.columns],style_cell={'textAlign': 'left'}),style={'width': '60vw','margin':'auto'}),df.to_dict('records')
...
def func(n_clicks):
    return dcc.send_data_frame(l_pred.to_csv, "Global.csv")
...
def func_spe(n_clicks,data,val):
    data_s = pd.DataFrame(data)
    return dcc.send_data_frame(data_s.to_csv, f"{val}.csv")
